...
<div class="p-1">
    ...
    <h1 class="text-dark p-3"
        ...
        Anecdotal Records of {{ $student_cha->lastname }}, {{ $student_cha->firstname }} from
        ...
        <div class="card">
            <div class="card-header text-center">
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ url('show-student-charity/' . $student_cha->id . '/anecdotal_record_charity/create/') }}"
                ...
                <table class="table table-striped  table-sm text-dark text-center bg-light">
                    ...
                    <tbody>
                        @forelse ($anecdotal_charity as $anec_cha)
                            <tr>
                                <td class="text-dark">{{ $anec_cha->observation_date_time->format('F d,  Y - g:i A') }}</td>
                                <td class="text-dark">{{ $anec_cha->location_of_incidents }}</td>
                                <td><a href="{{ url('/show-student-charity/'.$anec_cha->student->id.'/anecdotal_record_charity/' . $anec_cha->id) }}"
                                        class="btn btn-xs "><i class="fas fa-search text-info"></i></a></td>
                                <td><a href="{{ url('delete_anecdotal_record_charity/' . $anec_cha->id) }}"
