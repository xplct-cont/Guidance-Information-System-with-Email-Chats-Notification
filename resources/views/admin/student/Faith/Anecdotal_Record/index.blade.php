...
<div class="p-1">
    ...
    <h1 class="text-dark p-3"
        ...
        Anecdotal Records of {{ $student_fai->lastname }}, {{ $student_fai->firstname }} from
        ...
        <div class="card">
            <div class="card-header text-center">
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ url('show-student-faith/' . $student_fai->id . '/anecdotal_record_faith/create/') }}"
                ...
                <table class="table table-striped table-sm text-dark text-center bg-light">
                    ...
                    <tbody>
                        @forelse ($anecdotal_faith as $anec_fai)
                            <tr>
                                <td class="text-dark">{{ $anec_fai->observation_date_time->format('F d,  Y - g:i A') }}</td>
                                <td class="text-dark">{{ $anec_fai->location_of_incidents }}</td>
                                <td><a href="{{ url('/show-student-faith/'. $anec_fai->student->id .'/anecdotal_record_faith/' . $anec_fai->id) }}"
                                        class="btn btn-xs "><i class="fas fa-search text-info"></i></a></td>
                                <td><a href="{{ url('delete_anecdotal_record_faith/' . $anec_fai->id) }}"
